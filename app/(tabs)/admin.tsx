...
// ---> YOUR BUNNY CDN HOSTNAME <---
const BUNNY_CDN_HOSTNAME = 'vz-18e8d553-419.b-cdn.net';
...
export default function AdminScreen() {
  ...
  const handleCancelTask = (taskId: string) => {
    if (uploadRefs.current[taskId]) {
      uploadRefs.current[taskId].abort();
  ...
  const uploadVideoToBunny = async (fileObj: any, taskId: string, dbTable: string, dbId: string, subUrl: string | null = null): Promise<void> => {
    ...
    updateTask(taskId, { message: 'Connecting to secure Bunny CDN...' });
    ...
    const backendRes = await fetch('/api/mux', { 
        ...
        body: JSON.stringify({ title: `Upload for ${dbTable}:${dbId}` }) 
    });
    ...
    const { data: bunnyData } = await backendRes.json();
    if (!bunnyData?.signature) throw new Error(`No secure upload signature provided by backend`);
    ...
    return new Promise((resolve, reject) => {
      const upload = new tus.Upload(fileToUpload, {
        ...
        headers: {
          ...
          LibraryId: String(bunnyData.libraryId),
        },
        metadata: {
          filetype: fileToUpload.type || 'video/mp4',
          title: `Upload for ${dbTable}:${dbId}`,
        },
        ...
        onProgress: function (bytesUploaded, bytesTotal) {
          ...
          updateTask(taskId, { progress: Math.round(Number(percentage)), message: `Uploading Video: ${Math.round(Number(percentage))}%` });
        },
        onSuccess: async function () {
          delete uploadRefs.current[taskId];
          
          if (subUrl) {
            updateTask(taskId, { message: 'Attaching Subtitles to Bunny...' });
            try {
              await fetch('/api/captions', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ videoId: bunnyData.videoId, subtitleUrl: subUrl })
              });
            } catch (err) {
              console.error("Subtitle attach failed:", err);
            }
          }

          const generatedVideoUrl = `https://${BUNNY_CDN_HOSTNAME}/${bunnyData.videoId}/playlist.m3u8`;
          await supabase.from(dbTable).update({ video_url: generatedVideoUrl }).eq('id', dbId);

          updateTask(taskId, { status: 'done', progress: 100, message: 'Upload Complete! Encoding on Bunny...' });
          resolve();
        }
      });
  ...
  const runMovieBackground = async (taskId: string, title: string, desc: string, cat: string, poster: any, backdrop: any, video: any, subtitle: any, featured: boolean) => {
    try {
      ...
      updateTask(taskId, { message: 'Uploading Images & Subs...' });
      ...
      await uploadVideoToBunny(video, taskId, 'movies', data.id, subUrl);
  ...
  const runEpisodeBackground = async (taskId: string, seriesId: string, seasonNum: number, episodeNum: number, epTitle: string, video: any, subtitle: any) => {
    try {
      ...
      await uploadVideoToBunny(video, taskId, 'episodes', data.id, subUrl);
    } catch (e: any) { updateTask(taskId, { status: 'error', message: e.message || 'Unknown Error' }); }
  };
  ...
  const handleMasterUpdate = async () => {
    ...
    try {
      ...
      if (editingMovie.type === 'Movie' && (editVideoFile || editSubtitleFile)) {
        ...
        uploadVideoToBunny(editVideoFile, taskId, 'movies', editingMovie.id, subUrl).catch(e => updateTask(taskId, { status: 'error', message: e.message }));
      }
      ...
      if (editingMovie.type === 'TV Series' && editEpisodes.length > 0) {
        const episodeUpdates = editEpisodes.map(async ep => {
          ...
          if (ep.newVideoFile || ep.newSubtitleFile) {
             ...
             uploadVideoToBunny(ep.newVideoFile, taskId, 'episodes', ep.id, epSubUrl).catch(e => updateTask(taskId, {status: 'error', message: e.message}));
          }
        });
  ...
  const filteredActiveMovies = activeMovies.filter(m => m.title.toLowerCase().includes(manageSearchQuery.toLowerCase()));
    ...
    <View style={styles.container}>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
        ...
        {activeSection === 'manage' && (
          <View>
            {!editingMovie && (
              <View style={[styles.searchBox, { width: '100%', marginBottom: 15 }]}>
                <Ionicons name="search" size={18} color="#888" style={{ marginRight: 10 }} />
                <TextInput 
                  style={styles.searchInput} 
            ...
            {manageLoading ? <ActivityIndicator color="#e50914" /> : (
              <>
                {editingMovie ? (
                  ...
                  filteredActiveMovies.length === 0 ? (
                    <Text style={{color: '#888', padding: 10, textAlign: 'center'}}>No matching titles found.</Text>
                  ) : (
                    filteredActiveMovies.map(m => (
